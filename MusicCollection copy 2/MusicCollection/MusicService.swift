...
class MusicService {
    ...
    // MARK: - Methods
    func searchForMusic(songName: String, completion: @escaping (MusicSearchResponse?, Error?) -> ()) {
        let url = URL(string: "https://itunes.apple.com/search?term=" + songName + "&entity=song")!
    ...
    func pause(track: Track) {
        ...
        switch isDownloaded(track: track) {
        case true:
            print("pause from file")
            playerItem = .init(url: getFileUrl(for: track))
        case false:
            print("pause from url")
            playerItem = .init(url: track.previewUrl)
        }
